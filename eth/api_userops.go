...
func (api *composeUserOpsAPI) BuildSignedUserOpsTx(
    ...
    for i, op := range userOps {
        ...
        if minUserTip == nil || uTip.Cmp(minUserTip) < 0 { minUserTip = new(big.Int).Set(uTip) }
        if minUserFeeCap == nil || uFeeCap.Cmp(minUserFeeCap) < 0 { minUserFeeCap = new(big.Int).Set(uFeeCap) }
    }
