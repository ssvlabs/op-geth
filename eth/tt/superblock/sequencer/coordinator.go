...
func (sc *SequencerCoordinator) handleRequestSeal(ctx context.Context, from string, requestSeal *pb.RequestSeal) error {
	...
	// Seal and submit block (protocol-level submission via coordinator)
...
// GetIncludedXTsForCurrentSlot returns decided-included xtIDs (as pb.XtID) for the current slot
// Removed method which required external interface change; Eth layer records RequestSeal instead.
