...
func (b *EthAPIBackend) StartCallbackFn(chainID *big.Int) spconsensus.StartFn {
	// In SBCP, the Shared Publisher is the sole initiator (StartSC).
	// Followers initialize local 2PC state on StartSC, but must NOT re-send xtRequests to SP.
	return func(ctx context.Context, from string, xtReq *rollupv1.XTRequest) error { return nil }
}
